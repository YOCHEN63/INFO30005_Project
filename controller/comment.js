...
const reqComment = async (req, res, next) => {
    ...
    var result = bgl_comments
        .concat(exercise_comments)
        .concat(weight_comments)
        .concat(insulin_comments)
        ...
        for (i = 0; i < result.length; i++) {
            Object.assign(result[i], {
                record_date: new Intl.DateTimeFormat('en-AU', options).format(
                    result[i].record_date
                ),
            })
        }
    }
    res.render('clinician_comments_homepage', {
        all_comments: result,
        docData: docData,
        flash: req.flash('msg'),
    })
